...
$.fn.MSCalendar = function(k_date){
	//重写k_date的参数，把所有的值初始化
	k_date = {
		isDisabled : k_date.isDisabled || '0',
		maxDay : k_date.maxDay || 60,
	...
	$(this).live('click',function(e){
		//$('#popup_frame,#popup_pane,#a_tips_frame,#air_down_tips').hide(); 貌似无用
		kDate = $(this);
		kui_date();
	...
	**/
	function kui_date(){ //var d = new Date().getTime();
		...
		var kui_hours = kui_dd.getHours();//貌似无用
		var kui_minutes = kui_dd.getMinutes();//貌似无用
		var kui_seconds = kui_dd.getSeconds();//貌似无用
		var n_time = kui_dd.getTime();
		var max_date = new Date(n_time + (k_date.maxDay - 1)*24*60*60*1000);
		var vals = kDate.val();
		...
		**/
		...
		function change_date(dir){ 
			...
			var this_max_date = kui_max_date[kui_m-1]; //当前月总天数
			...
			for(var i=0;i<kui_td_lines;i++){
				...
				var dd3 = dateFormat(max_date); //拼接最大日期
				//先拼接第一行
				if(i === 0){
					// 第一行中有空白的单元格
					...
					for(var j=bb+1;j<=7;j++){
						...
						if(dd2 <= dd3 && dd2 >= dd1){
							...
							if(k_date.isDisabled == '1'){	//当前日期之前日期可用的情况
								if(vals === ''){
									cla = 'kui_not_kong';
									if(kui_d == d_ && dir == 'left'){
										cla = 'kui_not_kong td_select';
					...
					$('.kui_top_tr').removeClass('kui_top_tr'); //貌似无用
				}
				else if(i===kui_td_lines-1){ //再拼接最后一行
					var kui_i = 8-bb;
					for(var j=1;j<=7;j++){
						var dd2 = kui_y+'-'+kui_m+'-'+(7*(i-1)+kui_i);
						var cla = '';
						if((7*(i-1)+kui_i) > this_max_date){ //结尾的空格
							arr_tr.push('<dt class="kui_td_kong">&nbsp;</dt>');
						}
						else{
							if (dd2 > dd3) {
